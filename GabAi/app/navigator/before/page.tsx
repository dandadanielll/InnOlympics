...
const MOCK_CARE_PLAN = {
  ...
  riskRationale:
    'Symptoms suggest a non-urgent condition. The patient can wait for a scheduled clinic visit. Attending promptly is still recommended — worsening symptoms should prompt immediate reassessment.',
  ...
  script:
    'Doc, nagpapacheck-up po kami para sa aking anak na 4 na taong gulang. Lagnat po siya ng 3 araw... Walang kahirapan sa paghinga.',
  keyPoints: ['Fever duration: 3 days', 'Temperature peak: 38.5°C'],
}

export default function BeforePage() {
  const router = useRouter()
  const { createEncounter, updateEncounter } = useGabAiStore()
  ...
  const handleGeneratePlan = () => {
    // Create encounter in store and save the plan + script
    const id = createEncounter()
    updateEncounter(id, {
      symptoms: input,
      script: MOCK_CARE_PLAN.script,
      phase: 'during',
    })
    setShowPlan(true)
  }

  const handleGoToDuring = () => {
    router.push('/navigator/during')
  }
    ...
    <div style={{ maxWidth: 'var(--content-max)', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '48px' }}>
        ...
        <p className="text-body text-secondary" style={{ maxWidth: '600px' }}>
          Describe your concern below. GabAi will process your symptoms and generate a complete care plan — a routed
          facility, a documents checklist, and a doctor consultation script.
        </p>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button
                ...
                onMouseUp={() => {
                  ...
                  setInput('Lagnat ang anak ko ng 3 araw. Nasa QC kami, may PhilHealth kami. Ubo din siya at sipon.')
                }}
              >
              ...
              <div className="card">
                <div className="card-body">
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                    ...
                    <span className="text-sm" style={{ fontWeight: 600 }}>{MOCK_CARE_PLAN.riskLabel}</span>
                  </div>
                  ...
                  <button className="btn btn-secondary" style={{ width: '100%' }}>Open in Google Maps</button>
                </div>
              </div>
                ...
                {MOCK_CARE_PLAN.documents.map((d, i) => (
                  <div
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      padding: '12px 0',
                      borderBottom: i < MOCK_CARE_PLAN.documents.length - 1 ? '1px solid var(--border-light)' : 'none',
                    }}
                  >
                    <button
                      onClick={() => toggle(i)}
            ...
            <div className="card" style={{ borderTop: '4px solid var(--primary)' }}>
              <div className="card-body" style={{ background: 'var(--primary-light)' }}>
                <p style={{ fontFamily: 'Playfair Display, serif', fontStyle: 'italic', fontSize: '1.25rem', lineHeight: 1.8, color: 'var(--text-primary)', marginBottom: '20px' }}>
                  &ldquo;{MOCK_CARE_PLAN.script}&rdquo;
                </p>
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  ...
                  <button className="btn btn-primary" style={{ marginLeft: 'auto' }} onClick={handleGoToDuring}>
                    Pumunta sa During Phase →
                  </button>
                </div>
              </div>
            </div>
