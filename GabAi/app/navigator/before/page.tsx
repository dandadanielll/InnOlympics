...
export default function BeforePage() {
  ...
  const { currentEncounterId, createEncounter, updateEncounter, getCurrentEncounter, getLatestEncounter } = useGabAiStore()
  ...
  useEffect(() => {
    ...
    if (!encounter) {
      ...
      if (latest && (latest.phase === 'before' || latest.phase === 'during')) {
        // If they were in 'during', let them see 'before' again if they navigated here
        useGabAiStore.getState().setCurrentEncounter(latest.id)
        encounter = latest
      } else {
        createEncounter()
        return
    ...
    // 2. Hydrate local state from store
    if (encounter) {
      ...
      if (encounter.userLocation) {
        ...
        setLocationText(encounter.userLocation.address || 'Pinned Location')
        setLocationSet(true)
      }
  ...
  const handleChecklistChange = (checked: Record<string, boolean>) => {
    if (currentEncounterId) {
      updateEncounter(currentEncounterId, { checklist: checked })
    }
  }
  ...
  const handleStartJourney = () => {
    if (currentEncounterId) {
      updateEncounter(currentEncounterId, { phase: 'during' })
  ...
  const handleGoToStep3 = async () => {
    ...
    if (currentEncounterId) {
      updateEncounter(currentEncounterId, {
        selectedFacility,
        userLocation: { lat: userLat, lng: userLng, address: locationText }
      })
    }
      ...
      <section className={`bfr-sec ${completedStep3 ? '' : 'locked'}`} ref={step4Ref} style={{ height: 'auto', minHeight: '100vh', overflowY: 'auto', paddingBottom: '60px' }}>
        ...
        <div className="bfr-main" style={{ overflow: 'visible' }}>
          ...
          <DocumentChecklist
            ...
            initialCheckedDocs={getCurrentEncounter()?.checklist}
